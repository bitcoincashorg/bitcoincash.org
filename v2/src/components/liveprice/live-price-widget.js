...
const LivePriceWidget = ({ currentUSDPrice, currentEURPrice, currentJPYPrice, currentCNYPrice }) => {
  ...
  return (
    <div className={livePricingStyles.widgetContainer}>
        <div onClick={() => setOpen(!open)} className={livePricingStyles.widget}>
          {selectedCurrency}<span className={hide ? `${livePricingStyles.hide}` : "" }>{currentUSDPrice}</span><Triangle />
        </div>
